...
def run(args):
    ...
    with tqdm(total=len(filtered_messages)) as pbar:
        for msg in filtered_messages:
            ...
            df_data['question'].append(id2msg[msg['reply_to_message_id']]['text'].replace(',', ''))
            pbar.update(1)
    ...
    frequent_words = sorted(
            vocabulary.items(), reverse=True, key=lambda x: x[1])[:30]
